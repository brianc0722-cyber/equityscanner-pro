"""
Core asynchronous data ingestion layer (Render-safe).
aiokafka is fully optional.
"""
...
class AsyncDataIngestor:
    ...
    async def __aenter__(self):
        ...
        if AIOKAFKA_AVAILABLE and config.KAFKA_BOOTSTRAP_SERVERS and config.KAFKA_BOOTSTRAP_SERVERS != "localhost:9092":
            try:
    ...
    async def fetch_bars(self, ticker: str, start: datetime, end: datetime, interval: str = "1Min") -> List[Bar]:
        ...
        params = {"start": start.isoformat(), "end": end.isoformat(), "timeframe": interval, "limit": 10000}
        async with self.session.get(url, headers=headers, params=params) as resp:
            data = await resp.json()
        bars = []
    ...
    async def start_alpaca_stream(self, tickers: List[str]):
        ...
        async with websockets.connect(uri, extra_headers=headers) as ws:
            ...
            await ws.send(json.dumps({"action": "auth", "key": config.ALPACA_API_KEY, "secret": config.ALPACA_API_SECRET}))
            await ws.send(json.dumps({"action": "subscribe", "trades": tickers, "quotes": tickers}))
            async for message in ws:
                data = json.loads(message)
                if data.get("T") == "t":
                    trade = Trade(ticker=data["S"], price=data["p"], size=data["s"],
                                  timestamp=datetime.fromisoformat(data["t"].replace("Z", "+00:00")),
                                  conditions=data.get("c", []))
                    await self._publish("trade", trade.__dict__)
                elif data.get("T") == "q":
                    quote = Quote(ticker=data["S"], bid=data["bp"], ask=data["ap"],
                                  bid_size=data["bs"], ask_size=data["as"],
                                  timestamp=datetime.fromisoformat(data["t"].replace("Z", "+00:00")))
